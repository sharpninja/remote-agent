...
/// <remarks>Reads <see cref="PluginsOptions.Assemblies"/> and loads each assembly; exported types implementing <see cref="IAgentRunner"/> are instantiated via the service provider and registered by full type name. The "process" and "copilot-windows" runners are always included.</remarks>
/// <example><code>
...
{
    /// <summary>Builds a registry of named runners: "process" (default), "copilot-windows" (Copilot CLI on Windows), plus any from <see cref="PluginsOptions.Assemblies"/>.</summary>
    /// <param name="serviceProvider">Used to resolve <see cref="PluginsOptions"/> and to create plugin runner instances.</param>
    /// <returns>Dictionary of runner id to <see cref="IAgentRunner"/> (e.g. "process", "copilot-windows", or full type name of plugin type).</returns>
    public static IReadOnlyDictionary<string, IAgentRunner> BuildRunnerRegistry(IServiceProvider serviceProvider)
    {
        ...
        var dict = new Dictionary<string, IAgentRunner>(StringComparer.OrdinalIgnoreCase)
        {
            ...
            [CopilotWindowsAgentRunner.RunnerId] = serviceProvider.GetRequiredService<CopilotWindowsAgentRunner>()
        };
