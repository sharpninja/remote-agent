...
public sealed class UnbanPeerHandler(IServerCapacityClient client)
    : IRequestHandler<UnbanPeerRequest, CommandResult>
{
    public async Task<CommandResult> HandleAsync(UnbanPeerRequest request, CancellationToken cancellationToken = default)
    {
        var ok = await client.UnbanPeerAsync(
            request.Host, request.Port, request.Peer, request.ApiKey, cancellationToken);

        request.Workspace.StatusText = ok ? $"Peer unbanned: {request.Peer}" : $"Failed to unban peer: {request.Peer}";
